...
MultiskewBART <- function(X = X_norm, Y = Y_scaled, test_X = test_X_norm, tree_num=20, iter= 100, burn =50){ 
  ...
  EST_test_Y <- t( t(EST_mu) + sqrt(2/pi) * EST_Lam )
  ...
  return(list(y_hat_train = EST_test_Y, y_hat_test = EST_test_Y, Sigma = skew_Sig, lambda = EST_Lam))
}
